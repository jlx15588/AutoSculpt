...
class MultiStageGCNConv(MessagePassing):
    def __init__(self, in_channels, out_channels):
        super(MultiStageGCNConv, self).__init__(aggr='add')  # "Add" aggregation (Step 5).
    ...
    def forward(self, x, edge_index, edge_features):
        ...
        loop_edge_features = edge_features.new_full(
            (edge_index.shape[1] - edge_features.shape[0], edge_features.shape[1]), 1
        )
    ...
    def message(self, x_j, norm, edge_features):
        ...
        x_j = self.linear(x_j * edge_features)  # element wise product for node feature and edge feature
